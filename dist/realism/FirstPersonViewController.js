...
export class FirstPersonViewController {
  constructor(camera){this.camera=camera;this.yaw=0;this.pitch=-.015;this.targetYaw=0;this.targetPitch=-.015;this.looking=false;this.dragging=false;this.last=null;this.clock=0;this.lastLook=-10;this.neckLimit=2.62;this.gaze=new ObservedGaze();this.tracking=true;}
  center(){this.targetYaw=0;this.targetPitch=-.015;this.lastLook=this.clock;}
  ...
  update(dt,state,foot,hand,observation={}){this.clock+=dt;const gp=globalThis.navigator?.getGamepads?.()?.find(p=>p?.mapping==='standard');if(gp&&Math.hypot(gp.axes[2]||0,gp.axes[3]||0)>.18)this.move((gp.axes[2]||0)*dt*360,(gp.axes[3]||0)*dt*360);
    ...
    if(this.tracking&&gaze&&!this.looking&&!this.dragging&&this.clock-this.lastLook>1&&(['runup','flight'].includes(state)||(state==='result'&&observation.contact&&this.gaze.time-this.gaze.contactAt<2))){yaw=gaze.yaw;pitch=gaze.pitch;}
