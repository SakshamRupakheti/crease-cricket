...
export class BowlingSpeedController {
  ...
  configure(values){const next={...this,...values};if(!['fixed','random','progressive'].includes(next.mode))throw new RangeError('Unknown speed mode');for(const key of ['kmh','min','max'])if(!Number.isFinite(next[key])||next[key]<70||next[key]>165)throw new RangeError('Speed must be 70–165 km/h');if(next.min>next.max)throw new RangeError('Minimum must not exceed maximum');Object.assign(this,next);}
  choose(seed,legalBalls=0){const random=rng(seed^0x53a9);let kmh=this.mode==='random'?this.min+random()*(this.max-this.min):this.kmh;if(this.mode==='progressive')kmh+=Math.floor(legalBalls/6)*this.ramp;if(this.vary)kmh+=(random()*2-1)*5;return clamp(kmh,70,165);}
