...
export class HumanShotController {
 ...
 if(input.stroke&&input.stroke!==this.lastStroke){this.lastStroke=input.stroke;this.phase=0;this.active=true;this.finished=false;this.latched={...(this.smoothAdapt||this.adaptation(input,profile))};this.impactLoss=0;}
 ...
 if(this.active){this.phase=Math.min(1,this.phase+dt/duration*(1-this.impactLoss));if(this.phase>=1){this.active=false;this.finished=true;input.stroke=null;}}
 const t=this.active?this.phase:0;let m=sampleMotion(profile.id,t);if(this.blend&&canBlend(profile.id,this.blend.id))m=blendMotion(m,sampleMotion(this.blend.id,t),clamp(this.blend.amount,0,1));
