...
export class BatController {
  constructor(){this.mode='mouse';this.sensitivity=1;this.invert=false;this.hand='right';this.target=v(.15,.72,.03);this.position={...this.target};this.face=0;this.loft=0;this.foot=v();this.stroke=null;this.clock=0;this.leave=false;this.lastPointer=null;this.wheelBurst=null;this.pointers=new Map();this.active=true;this.onActivity=()=>{};this.keys=new Set();this.lastStroke=-1;}
  ...
  step(dt){this.clock+=dt;const speed=.75*dt;if(this.keys.has('KeyA'))this.foot.x-=speed;if(this.keys.has('KeyD'))this.foot.x+=speed;if(this.keys.has('KeyW'))this.foot.z-=speed;if(this.keys.has('KeyS'))this.foot.z+=speed;this.foot.x=clamp(this.foot.x,-.4,.4);this.foot.z=clamp(this.foot.z,-.38,.22);if(this.keys.has('KeyQ'))this.face-=dt;if(this.keys.has('KeyE'))this.face+=dt;this.face=clamp(this.face,-.7,.7);let z=this.target.z;
    if(this.stroke){this.stroke.age+=dt;const a=this.stroke.age,duration=.32;if(a<duration)z-=Math.sin(Math.PI*a/duration)*(.45+this.stroke.power*.8);else this.stroke=null;}
    const alpha=1-Math.exp(-dt*45);this.position.x+=(this.target.x+this.foot.x-this.position.x)*alpha;this.position.y+=(this.target.y-this.position.y)*alpha;this.position.z+=(z+this.foot.z-this.position.z)*alpha;
    this.loft=(this.keys.has('ShiftLeft')||this.keys.has('ShiftRight'))?.42:0;
    return {position:{...this.position},yaw:this.face,loft:this.loft,leave:this.leave};
  }
